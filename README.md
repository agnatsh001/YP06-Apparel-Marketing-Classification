...
|---|---:|---:|---:|
| Logistic Regression (baseline) | 0.7651 | ±0.017 | — |
| LightGBM дефолт | 0.6836 | ±0.026 | — |
| LightGBM + Optuna | 0.7652 | ±0.017 | — |
| CatBoost + Optuna | 0.7702 | ±0.016 | — |
| **Стекинг (финал)** | **0.7731** | **±0.017** | **0.7799** |

**Финальная архитектура:** LogReg + LightGBM + CatBoost → мета-LogReg

**Калибровка вероятностей** (IsotonicRegression на OOF):

- До калибровки: `sum(predict_proba)` = 3 937 при фактических 192 покупателях (расхождение ×21)
- После калибровки: прогноз 187 покупателей, отклонение −2.7%
- ROC-AUC: 0.7799 → 0.7823 (ранжирование сохраняется с небольшим отклонением)
